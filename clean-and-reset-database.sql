...
-- 2단계: 인증 관련 데이터 정리 (Supabase Auth 테이블)
-- 기존 사용자 삭제 (테스트 계정들)
...
CREATE TABLE inquiries (
  ...
  user_id UUID REFERENCES profiles(id) ON DELETE SET NULL NULL,
...
CREATE POLICY "누구나 매입 신청을 할 수 있습니다" ON purchase_requests
  FOR INSERT WITH CHECK (true);
...
CREATE POLICY "누구나 프로필을 생성할 수 있습니다" ON profiles
...
-- 완료 메시지
SELECT '데이터베이스가 완전히 정리되고 재설정되었습니다.' as message;